...
export default async function ClubPage() {
  ...
  await supabase.from("profiles").update({ last_content_seen_at: seenAt }).eq("id", profile.id);
  nextProfile = {
    ...profile,
    ...
    <PrivateShell profile={nextProfile} admin={nextProfile.role === "admin"}>
      <section className="space-y-6">
        <div className="rounded-[32px] border border-white/10 bg-white/5 p-6">
          <p className="text-sm uppercase tracking-[0.28em] text-cyanGlow">Closed Club</p>
          <h2 className="mt-3 text-3xl font-semibold text-white">Лента закрытого клуба</h2>
          <p className="mt-3 max-w-2xl text-sm leading-7 text-white/62">
            Здесь собран закрытый контент по уровням доступа. Чем выше уровень, тем глубже лента.
          </p>
        </div>
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <h3 className="mt-2 text-2xl font-semibold text-white">
                  Комментарии под материалами клуба
                </h3>
              </div>
        ...
        <div className="mx-auto max-w-5xl space-y-5">
          {postsWithThumbnails.length ? (
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6 text-white/60">
              Пока в закрытом клубе нет материалов.
            </div>
          )}
        </div>
      </section>
