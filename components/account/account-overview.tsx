  ...
  return <div className="grid gap-5 lg:grid-cols-[0.82fr_1.18fr]">
    ...
    <section id="subscription" className="scroll-mt-6 rounded-[30px] border border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(255,79,216,0.14),transparent_34%),rgba(255,255,255,0.04)] p-5 shadow-glow sm:p-6">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        ...
        {tier > 0 && tier < 4 ? <Link href={"/tg/tiers" as Route} className="inline-flex items-center justify-center rounded-2xl border border-white/12 bg-white/[0.05] px-4 py-3 text-sm font-semibold text-white transition hover:bg-white/[0.09]">{copy.upgrade}</Link> : null}
        <Link href={"/tg/tiers" as Route} className="inline-flex items-center justify-center rounded-2xl border border-white/12 bg-white/[0.05] px-4 py-3 text-sm text-white/78 transition hover:text-white">{copy.manage}</Link>
