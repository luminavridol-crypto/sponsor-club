    ...
    <section className={ADMIN_PANEL_CLASS}>
      ...
      <div className="relative">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h3 className="font-display text-[1.5rem] font-semibold text-white">Дни рождения</h3>
          </div>
          <div className="flex gap-2 self-end sm:self-auto">
            <button type="button" onClick={goToPreviousMonth} className={ADMIN_BUTTON_SECONDARY_CLASS}>
              ←
            </button>
            <button type="button" onClick={goToNextMonth} className={ADMIN_BUTTON_SECONDARY_CLASS}>
              →
            </button>
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
          <div>
            <div className="grid gap-3 sm:grid-cols-[1fr_120px]">
              <div className="rounded-[20px] border border-white/10 bg-black/18 px-4 py-3 text-sm capitalize text-white sm:text-base">
                {getMonthName(selectedMonth)} {selectedYear}
              </div>
              <select
                value={selectedYear}
                onChange={(event) => setSelectedYear(Number(event.target.value))}
                className="rounded-[20px] border border-white/10 bg-black/18 px-4 py-3 text-sm text-white outline-none sm:text-base"
              >
                {years.map((year) => (
            ...
            <div className="mt-5 grid grid-cols-7 gap-1.5 text-center sm:gap-2">
              ...
              {calendarDays.map((day, index) => (
                ...
                >
                  ...
                  {day.birthdayCount > 0 ? <div className="mt-1 text-[11px] text-fuchsia-100">{day.birthdayCount} д.р.</div> : null}
                </div>
              ))}
            </div>
          </div>
              ...
              <div className="mt-3 space-y-2">
                {upcomingBirthdays.length ? (
                  upcomingBirthdays.map((person) => (
                    <div key={`upcoming-${person.id}`} className={`flex items-center justify-between gap-3 rounded-[18px] border px-4 py-3 ${getBirthdayRowClass(person.tierKey)}`}>
                  ...
                  <div className="rounded-[18px] border border-white/10 px-4 py-3 text-sm text-white/55">
              ...
              <div className="mt-3 space-y-2">
                {monthBirthdays.length ? (
                  monthBirthdays.map((person) => {
                    ...
                    return (
                      <div key={person.id} className={`flex items-center justify-between gap-3 rounded-[18px] border px-4 py-3 ${getBirthdayRowClass(person.tierKey)}`}>
                        <div className="min-w-0">
                          <p className="truncate font-medium text-white">{person.displayName}</p>
                          <p className={`text-sm ${getBirthdayTierTextClass(person.tierKey)}`}>{person.tierLabel}</p>
                        </div>
