...
async function createToken(payload: UploadWorkerTokenPayload, secret: string) {
...
function validateObjectKey(objectKey: string, tokenPayload: UploadWorkerTokenPayload) {
  if (!objectKey || tokenPayload.objectKey !== objectKey) {
    throw new Error("Upload token does not match object key.");
  }
}

function validateUploadId(uploadId: string, tokenPayload: UploadWorkerTokenPayload) {
...
function parseParts(value: unknown) {
  ...
  const parts = value.map((part) => {
    const partNumber = parsePositiveInt((part as { partNumber?: unknown }).partNumber);
